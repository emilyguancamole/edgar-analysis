...
# psql -d edgar_db -c "SELECT accession_number, report_date, primary_doc_url FROM filings WHERE accession_number IN (SELECT DISTINCT accession_number FROM staging_13f) ORDER BY accession_number LIMIT 10;"
## example join (filings -> holdings_raw -> issuers) for sample accession:
# psql -d edgar_db -c "SELECT f.accession_number, i.cusip, i.issuer_name, h.shares_owned, h.share_type FROM filings f JOIN holdings_raw h ON f.filing_id = h.filing_id JOIN issuers i ON i.issuer_id = h.issuer_id WHERE f.accession_number = '000108514625004804' LIMIT 5;"
