...
def merge_13f_staging_to_schema(staging_table, conn):
    with conn.cursor() as cur:
        # Insert securities, dedup on cusip
        cur.execute(f"""
            INSERT INTO securities (issuer_name, cusip, figi)
        ...
        # print fund_id for debug:
        cur.execute(f"SELECT DISTINCT cik FROM {staging_table} LIMIT 5;")
        sample_ciks = [row[0] for row in cur.fetchall()]
        print("DEBUG: Sample CIKs from staging_13f:", sample_ciks)
        ...
        cur.execute(f"""
            ...
            JOIN securities i ON i.cusip = s.cusip
...
def merge_13g_staging_to_schema(staging_table, conn):
    ...
    with conn.cursor() as cur:
        # Insert securities, dedup on cusip
        cur.execute(f"""
            INSERT INTO securities (issuer_name, cusip)
            SELECT DISTINCT issuer, cusip
            FROM {staging_table}
            ON CONFLICT (cusip) DO NOTHING;
        """)
        conn.commit()
        print("securities rows:", _count(cur, "securities"))
...
def build_holdings_ts(conn):
    ...
    # Check for NULL fund_id before processing
    if df['fund_id'].isnull().any():
        missing = df[df['fund_id'].isnull()].head(10)
        raise RuntimeError(f"Found rows with NULL fund_id in holdings_ts build — sample:\n{missing.to_string()}")
...
if __name__ == "__main__":
    conn = get_conn()
    # load_funds(conn) # this is done in merging (savers.py)
